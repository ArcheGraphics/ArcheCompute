...
struct HashGrid {
    ...
    device int *point_cells{nullptr};// cell id of a point
    device int *point_ids{nullptr};  // index to original point

    device int *cell_starts{nullptr};// start index of a range of indices belonging to a cell, dim_x*dim_y*dim_z in length
    device int *cell_ends{nullptr};  // end index of a range of indices belonging to a cell, dim_x*dim_y*dim_z in length
...
inline int hash_grid_index(const thread HashGrid &grid, int x, int y, int z) {
...
inline int hash_grid_index(const thread HashGrid &grid, const thread vec3 &p) {
...
inline hash_grid_query_t hash_grid_query(uint64_t id, vec3 pos, float radius) {
    hash_grid_query_t query;

    query.grid = *(const device HashGrid *)(id);
...
inline bool hash_grid_query_next(thread hash_grid_query_t &query, int index) {
    const thread HashGrid &grid = query.grid;
    if (!grid.point_cells)
        return false;
...
inline int iter_next(thread hash_grid_query_t &query) {
    return query.current;
}

inline bool iter_cmp(thread hash_grid_query_t &query) {
    bool finished = hash_grid_query_next(query, query.current);
    return finished;
}

inline hash_grid_query_t iter_reverse(const thread hash_grid_query_t &query) {
    // can't reverse grid queries, users should not rely on neighbor ordering
    return query;
}

inline int hash_grid_point_id(uint64_t id, int index) {
    const device HashGrid *grid = (const device HashGrid *)(id);
