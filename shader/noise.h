...
inline float smoothstep_gradient(float t) {
    return 6.f * t * (1.f - t);
}
...
inline vec2 interpolate_gradient_2d(float a0, float a1, float t, thread vec2 &d_a0, thread vec2 &d_a1, thread vec2 &d_t) {
    return (d_a1 - d_a0) * smootherstep(t) + (a1 - a0) * smootherstep_gradient(t) * d_t + d_a0;
}

inline vec3 interpolate_gradient_3d(float a0, float a1, float t, thread vec3 &d_a0, thread vec3 &d_a1, thread vec3 &d_t) {
    return (d_a1 - d_a0) * smootherstep(t) + (a1 - a0) * smootherstep_gradient(t) * d_t + d_a0;
}

inline vec4 interpolate_gradient_4d(float a0, float a1, float t, thread vec4 &d_a0, thread vec4 &d_a1, thread vec4 &d_t) {
    return (d_a1 - d_a0) * smootherstep(t) + (a1 - a0) * smootherstep_gradient(t) * d_t + d_a0;
}

inline float random_gradient_1d(uint32_t state, int ix) {
    const uint32_t p1 = 73856093;
    uint32_t idx = ix * p1 + state;
    return randf(idx, -1.f, 1.f);
}

inline vec2 random_gradient_2d(uint32_t state, int ix, int iy) {
    const uint32_t p1 = 73856093;
...
inline vec3 random_gradient_3d(uint32_t state, int ix, int iy, int iz) {
    const uint32_t p1 = 73856093;
    const uint32_t p2 = 19349663;
...
inline vec4 random_gradient_4d(uint32_t state, int ix, int iy, int iz, int it) {
    const uint32_t p1 = 73856093;
    const uint32_t p2 = 19349663;
    const uint32_t p3 = 53471161;
...
inline float dot_grid_gradient_1d(uint32_t state, int ix, float dx) {
    float gradient = random_gradient_1d(state, ix);
    return dx * gradient;
}

inline float dot_grid_gradient_2d(uint32_t state, int ix, int iy, float dx, float dy) {
    vec2 gradient = random_gradient_2d(state, ix, iy);
    return (dx * gradient[0] + dy * gradient[1]);
}

inline float dot_grid_gradient_3d(uint32_t state, int ix, int iy, int iz, float dx, float dy, float dz) {
    vec3 gradient = random_gradient_3d(state, ix, iy, iz);
    return (dx * gradient[0] + dy * gradient[1] + dz * gradient[2]);
}

inline float dot_grid_gradient_4d(uint32_t state, int ix, int iy, int iz, int it, float dx, float dy, float dz, float dt) {
    vec4 gradient = random_gradient_4d(state, ix, iy, iz, it);
    return (dx * gradient[0] + dy * gradient[1] + dz * gradient[2] + dt * gradient[3]);
}

inline float noise_1d(uint32_t state, int x0, int x1, float dx) {
    //vX
...
inline float noise_1d_gradient(uint32_t state, int x0, int x1, float dx) {
...
inline float noise_2d(uint32_t state, int x0, int y0, int x1, int y1, float dx, float dy) {
    //vXY
...
inline vec2 noise_2d_gradient(uint32_t state, int x0, int y0, int x1, int y1, float dx, float dy) {
    vec2 d00 = vec2(dx, dy);
...
inline float noise_3d(uint32_t state, int x0, int y0, int z0, int x1, int y1, int z1, float dx, float dy, float dz) {
    //vXYZ
...
inline vec3 noise_3d_gradient(uint32_t state, int x0, int y0, int z0, int x1, int y1, int z1, float dx, float dy, float dz) {
...
inline float noise_4d(uint32_t state, int x0, int y0, int z0, int t0, int x1, int y1, int z1, int t1, float dx, float dy, float dz, float dt) {
    //vXYZT
...
inline vec4 noise_4d_gradient(uint32_t state, int x0, int y0, int z0, int t0, int x1, int y1, int z1, int t1, float dx, float dy, float dz, float dt) {
...
inline float noise(uint32_t state, float x) {
    float dx = x - floor(x);
...
inline float noise(uint32_t state, const thread vec2 &xy) {
...
inline float noise(uint32_t state, const thread vec3 &xyz) {
...
inline float noise(uint32_t state, const thread vec4 &xyzt) {
...
inline float pnoise(uint32_t state, float x, int px) {
    float dx = x - floor(x);
...
inline float pnoise(uint32_t state, const thread vec2 &xy, int px, int py) {
    float dx = xy[0] - floor(xy[0]);
    float dy = xy[1] - floor(xy[1]);
...
inline float pnoise(uint32_t state, const thread vec3 &xyz, int px, int py, int pz) {
    float dx = xyz[0] - floor(xyz[0]);
    float dy = xyz[1] - floor(xyz[1]);
    float dz = xyz[2] - floor(xyz[2]);
...
inline float pnoise(uint32_t state, const thread vec4 &xyzt, int px, int py, int pz, int pt) {
    float dx = xyzt[0] - floor(xyzt[0]);
    float dy = xyzt[1] - floor(xyzt[1]);
    float dz = xyzt[2] - floor(xyzt[2]);
    float dt = xyzt[3] - floor(xyzt[3]);
...
inline vec2 curlnoise(uint32_t state, const thread vec2 &xy,
                      const uint32_t octaves, const float lacunarity, const float gain) {
...
inline vec3 curlnoise(uint32_t state, const thread vec3 &xyz,
                      const uint32_t octaves, const float lacunarity, const float gain) {
...
inline vec3 curlnoise(uint32_t state, const thread vec4 &xyzt,
                      const uint32_t octaves, const float lacunarity, const float gain) {
